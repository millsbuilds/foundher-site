            ...
            <div key={p.name} style={{ background: C.white, overflow: "hidden", ...(p.name === "The Beach Towel" ? { gridColumn: "1 / -1" } : {}) }}>
              {p.image2 ? (
                <div style={{ display: "flex", width: "100%", height: p.imageHeight ?? 400, gap: 8 }}>
                  <img src={p.image} alt={`${p.name} product`} style={{ flex: `1 1 ${p.imageWidths?.[0] ?? "50%"}`, minWidth: 0, height: "100%", objectFit: p.imageFit ?? "cover", objectPosition: (p as any).imagePosition ?? "center", display: "block", backgroundColor: p.imageBg }} />
                  {p.name === "The Beach Towel" ? (
                    ...
                    <img src={p.image2} alt={`${p.name} lifestyle`} style={{ flex: `1 1 ${p.imageWidths?.[1] ?? "50%"}`, minWidth: 0, height: "100%", objectFit: p.imageFit ?? "cover", objectPosition: (p as any).imagePosition2 ?? "center", display: "block", backgroundColor: p.imageBg }} />
                  )}
                </div>
              ) : (
